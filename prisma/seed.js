...
const client_1 = require("@/infra/database/orms/prisma/client");
...
const seedPermissionRules = async () => {
    ...
    for (const permissionRule of permissionRules) {
        const hasPermission = await client_1.prisma.permissionRule.findFirst({
            where: { rule: permissionRule.rule },
        });
        if (!hasPermission) {
            await client_1.prisma.permissionRule.create({ data: permissionRule });
        }
    }
};
const seedPermissionGroups = async () => {
    ...
    for (const permissionGroup of permissionGroups) {
        const hasPermission = await client_1.prisma.permissionGroup.findFirst({
            where: { name: permissionGroup.name },
        });
        if (!hasPermission) {
            await client_1.prisma.permissionGroup.create({ data: permissionGroup });
        }
    }
};
const seedPermissionGroupsRules = async () => {
    ...
    for (const permissionGroupRule of permissionGroupsRules) {
        const hasPermission = await client_1.prisma.permissionGroupRule.findFirst({
            where: {
        ...
        if (!hasPermission) {
            await client_1.prisma.permissionGroupRule.create({ data: permissionGroupRule });
        }
    }
};
const seedUsers = async () => {
    const hashedPassword = await (0, hash_1.generateHash)('01020304');
    const users = [
        {
    ...
    for (const user of users) {
        const hasPermission = await client_1.prisma.user.findFirst({
            where: { email: user.email },
        });
        if (!hasPermission) {
            await client_1.prisma.user.create({ data: user });
        }
    }
};
const seedUsersPermissionGroup = async () => {
    ...
    for (const userPermissionGroup of usersPermissionGroup) {
        const hasPermission = await client_1.prisma.userPermissionGroup.findFirst({
            where: {
        ...
        if (!hasPermission) {
            await client_1.prisma.userPermissionGroup.create({ data: userPermissionGroup });
        }
    }
};
const seed = async () => {
