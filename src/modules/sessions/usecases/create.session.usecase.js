...
let CreateSessionUseCase = (() => {
    ...
    var CreateSessionUseCase = class {
        ...
        static {
            ...
            CreateSessionUseCase = _classThis = _classDescriptor.value;
        ...
        async execute(authUser) {
            ...
            const isPassword = await (0, hash_1.verifyHash)(password, user.password);
            if (!isPassword) {
                throw new unauthorized_error_1.UnauthorizedError();
            }
            const token = app_1.app.jwt.sign({
                id: user.id,
                email: user.email,
                name: user.name,
            }, { expiresIn: '1h' });
    ...
    return CreateSessionUseCase = _classThis;
})();
